"""Copy text files under a directory to .py files in a new directory.
...
def convert(
    ...
    *,
    ...
    for source in iter_text_files(source_root):
        relative_path = source.relative_to(source_root)
        target = output_root / relative_path.with_suffix(".py")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Copy all .txt files in a directory tree to .py files."
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "sibling folder named SOURCE_py."
        ),
    )
    parser.add_argument(
        "--dry-run",
    ...
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Replace existing .py targets in the output directory.",
    )
...
def main() -> None:
    ...
    output_root = (
        ...
        else source_root.parent / f"{source_root.name}_py"
    )
